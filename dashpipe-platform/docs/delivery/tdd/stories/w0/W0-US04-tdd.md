...
| Stale `target/` after yaml/logback | `./mvnw -pl dashpipe-api clean test` |
...
| `StructuredLoggingSmokeTest` | (config smoke) | `logging.structured.format.console == "logstash"`; `spring.application.name == "dashpipe-api"`; `logback-spring.xml` on classpath |

Same Compose MySQL assume pattern as health IT for `PrometheusEndpointIT`.

```bash
./mvnw -pl dashpipe-api test -Dtest=PrometheusEndpointIT,StructuredLoggingSmokeTest
...
```bash
./mvnw -pl dashpipe-api clean test -Dtest=PrometheusEndpointIT,StructuredLoggingSmokeTest
...
- Keep metrics tags simple (`application: dashpipe-api`)
- Comment in logback that Wave 4 will extend ELK
- Re-run health + prometheus ITs together

```bash
./mvnw -pl dashpipe-api test -Dtest=HealthControllerIT,PrometheusEndpointIT,StructuredLoggingSmokeTest
```
...
| Stale `target/` after yaml change | `./mvnw -pl dashpipe-api clean test` |
| Logging DB password | Never log datasource password |

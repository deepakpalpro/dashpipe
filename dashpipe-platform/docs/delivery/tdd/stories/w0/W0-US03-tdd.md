...
```bash
./mvnw -pl dashpipe-api test -Dtest=FlywayBaselineIT
...
3. `dashpipe-api/src/main/resources/db/migration/V1__baseline.sql` — `CREATE TABLE tenants (...)`.
...
```bash
./mvnw -pl dashpipe-api test -Dtest=FlywayBaselineIT,HealthControllerIT
```

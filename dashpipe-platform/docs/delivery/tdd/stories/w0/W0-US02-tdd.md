...
| **You will touch** | parent `pom.xml`, `dashpipe-api/`, Actuator, `HealthControllerIT` |
...
|-----------|----------------|
| Parent `pom.xml` | Spring Boot 3.x, Java 21, module `dashpipe-api` |
| `dashpipe-api/pom.xml` | web, actuator, jdbc, mysql-connector-j, test |
...
```bash
docker compose up -d mysql
./mvnw -pl dashpipe-api test -Dtest=HealthControllerIT
...
1. Parent `pom.xml` — Spring Boot 3.x, Java 21, module `dashpipe-api`.
2. `dashpipe-api/pom.xml` — web, actuator, jdbc, mysql-connector-j, test.

...
**Done means:** `TenantFixturesTest` and `WireMockHarnessTest` green via `./mvnw -pl dashpipe-api test`.
...
| # | Assumption |
|---|------------|
| 1 | W0-US02 merged (`dashpipe-api` module exists) |
...
Path: `dashpipe-api/src/test/resources/fixtures/tenants/t001.json`

WireMock: `@RegisterExtension WireMockExtension` dynamic port; stub `GET /external/ping`; call `WIRE_MOCK.baseUrl() + "/external/ping"`.

```bash
./mvnw -pl dashpipe-api test -Dtest=TenantFixturesTest,WireMockHarnessTest
```
...
| # | Action | Expected |
|---|--------|----------|
| 1 | `./mvnw -pl dashpipe-api test -Dtest=TenantFixturesTest,WireMockHarnessTest` | Green |

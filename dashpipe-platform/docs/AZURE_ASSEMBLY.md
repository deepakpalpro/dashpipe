...
First cloud assembly for Dashpipe.
...
```bash
export DASHPIPE_MYSQL_URL='jdbc:mysql://<server>.mysql.database.azure.com:3306/dashpipe?useSSL=true'
export DASHPIPE_MYSQL_USER='dashpipe'
export DASHPIPE_MYSQL_PASSWORD='…'
export DASHPIPE_SERVICEBUS_CONNECTION_STRING='Endpoint=sb://…'
export DASHPIPE_SERVICEBUS_NAMESPACE='….servicebus.windows.net'
export DASHPIPE_IMAGE_TAG='0.1.0'
# Point Jobs at ACR, e.g.:
export PIPELINE_K8S_DEFAULT_IMAGE_PATTERN='<acr>.azurecr.io/dashpipe/{pipeletId}:0.1.0'

java -jar dashpipe-api.jar --spring.profiles.active=azure
```

Profile sets `dashpipe.platform.broker=servicebus` and excludes RabbitMQ auto-configuration.
...
| Images | `dashpipe/plet-*:local` | `$ACR/dashpipe/plet-*:tag` | same ACR pattern |

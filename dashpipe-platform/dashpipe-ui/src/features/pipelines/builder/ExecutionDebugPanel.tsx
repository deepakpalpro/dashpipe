...
export function ExecutionDebugPanel({
  ...
  const kubectlJobs = `kubectl get jobs,pods -n ${ns} -l dashpipe.io/execution_id=${executionId}`
  const kubectlLogs = `kubectl logs -n ${ns} -l dashpipe.io/execution_id=${executionId} --tail=100`

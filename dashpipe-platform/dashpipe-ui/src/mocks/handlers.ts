...
export const CONNECTOR_TYPES: ConnectorType[] = [
  {
    ...
    spiClass: 'com.dashpipe.connector.rest.RestConnector',
    spiVersion: '1.0',
  },
  {
    ...
    spiClass: 'com.dashpipe.connector.spi.Connector',
    spiVersion: '1.0',
  },
  {
    id: 'ct-storage',
    ...
    spiClass: 'com.dashpipe.connector.storage.StorageConnector',
    spiVersion: '1.0',
  },
  {
    id: 'ct-message-bus',
    ...
    spiClass: 'com.dashpipe.connector.messagebus.MessageBusConnector',
    spiVersion: '1.0',
  },
]
...
export function resetMockDb() {
  ...
  mockDb.pipelines = [
    ...
    {
      id: 'pipe-inventory-sync-15m',
      tenantId: 'T001',
      name: 'Inventory Sync every 15m',
      description:
        'Every 15 minutes: Schedule tick → pull inventory from petstore-inventory (:4011) → filter → update petstore (:4010).',
      visibility: 'PRIVATE',
      execution_mode: 'ASYNC',
      version: 1,
      status: 'ACTIVE',
      deployment_config: { cloud: 'local', region: 'us-east-1' },
      execution_config: {
        ioMode: 'queue',
        batchSize: '1',
        scheduleCron: '*/15 * * * *',
      ...
      steps: [
        {
          id: 'step-inv-1',
          pipelet_id: 'plet-schedule-source',
          step_order: 1,
          config: { cron: '*/15 * * * *', batchSize: '1' },
          deployment_config: { cloud: 'local', region: 'us-east-1' },
          execution_config: { cron: '*/15 * * * *', batchSize: '1' },
          connector_ids: [],
          service_ids: [],
          input_queue: null,
          output_queue: 'q-inv-1',
        },
        {
          id: 'step-inv-2',
          pipelet_id: 'plet-rest-source',
        ...
        },
        {
          ...
          deployment_config: { cloud: 'local', region: 'us-east-1' },
          execution_config: { expression: 'True' },
          connector_ids: [],
          service_ids: [],
          input_queue: 'q-inv-2',
          output_queue: 'q-inv-3',
        },
        {
          id: 'step-inv-4',
          pipelet_id: 'plet-webhook-destination',
          step_order: 4,
          config: { path: '/inventory/upload', method: 'POST' },
          deployment_config: { cloud: 'local', region: 'us-east-1' },
          execution_config: { path: '/inventory/upload', method: 'POST' },
          connector_ids: ['conn-plet-webhook-destination'],
          service_ids: [],
          input_queue: 'q-inv-3',
          output_queue: null,
        },
    ...
    {
      id: 'pipe-manual-trigger-s3-out',
      tenantId: 'T001',
      name: 'Manual Trigger → S3 Out',
      description:
        'Click Run: Manual Source emits a trigger payload → pass-through filter → S3 Destination writes JSON to LocalStack S3 (demo-file-destination).',
      visibility: 'PRIVATE',
      execution_mode: 'ASYNC',
      version: 1,
      status: 'DRAFT',
      deployment_config: { cloud: 'local', region: 'us-east-1' },
      execution_config: { ioMode: 'queue', batchSize: '1' },
      ...
      steps: [
        {
          ...
          config: {
            batchSize: '1',
            note: 'Emits one trigger record when you click Run (no schedule).',
          },
          deployment_config: { cloud: 'local', region: 'us-east-1' },
          execution_config: {
            batchSize: '1',
        ...
        },
        {
          ...
          connector_ids: [],
          service_ids: [],
          input_queue: 'q-manual-1',
          output_queue: 'q-manual-2',
        },
        {
          id: 'step-manual-3',
          pipelet_id: 'plet-s3-destination',
          step_order: 3,
          config: { objectKey: 'manual/trigger-out.json' },
          deployment_config: { cloud: 'local', region: 'us-east-1' },
          execution_config: { objectKey: 'manual/trigger-out.json' },
          connector_ids: ['conn-plet-s3-destination'],
          service_ids: [],
          input_queue: 'q-manual-2',
          output_queue: null,
        },
      ],
    },
...
export const pipelineHandlers = [
  ...
  http.post('/api/v1/pipelines/:id/run', ({ params, request }) => {
    ...
    mockDb.executions[executionId] = {
      ...
      steps: [
        ...
        { step_order: 2, pipelet_id: 'plet-json-transform', status: 'PENDING' },
        {
          step_order: 3,
          pipelet_id: 'plet-webhook-destination',
          status: 'PENDING',
  ...
  http.post(
    '/api/v1/pipelines/:id/executions/:executionId/cancel',
    ({ params, request }) => {
      const tid = tenantId(request)
      const pipelineId = String(params.id)
      const executionId = String(params.executionId)
      ...
      if (!terminal) {
        mockDb.executions[executionId] = {
          ...
          status: 'CANCELLED',
          completed_at: nowIso(),
          error_summary: '{"message":"Force stopped by user"}',

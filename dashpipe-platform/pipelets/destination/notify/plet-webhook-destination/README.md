...
```bash
docker build -f pipelets/plet-webhook-destination/Dockerfile -t dashpipe/plet-webhook-destination:local pipelets
echo '{"payload":{"mode":"upsert","items":[]}}' | docker run --rm -i --network dashpipe_default \
  -e IO_MODE=stdio \
  ...
  dashpipe/plet-webhook-destination:local
```

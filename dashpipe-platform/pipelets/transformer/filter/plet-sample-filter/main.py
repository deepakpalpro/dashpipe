#!/usr/bin/env python3
"""plet-sample-filter — generated runnable pipelet."""
...
def main() -> int:
    log("plet-sample-filter starting")
    ...
    if isinstance(payload, str) and payload.startswith("run-"):
        message = {}
    out = run(message, execution, _connector(), "plet-sample-filter")

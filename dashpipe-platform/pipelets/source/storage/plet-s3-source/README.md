...
```bash
docker build -f pipelets/plet-s3-source/Dockerfile -t dashpipe/plet-s3-source:local pipelets

docker run --rm --network dashpipe_default \
  -e IO_MODE=stdio \

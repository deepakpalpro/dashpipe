...
import com.dashpipe.api.k8s.PipeletJobClient;
import com.dashpipe.api.k8s.PipeletJobRequest;
import com.dashpipe.api.k8s.PipeletJobRequestFactory;
import com.dashpipe.api.messaging.PipelineTopology;
import com.dashpipe.api.messaging.PipelineTopologyService;
import com.dashpipe.api.messaging.QueueNaming;
import com.dashpipe.api.messaging.RabbitMessagingConfig;
import com.dashpipe.api.observability.CompletenessCalculator;
import com.dashpipe.api.observability.CompletenessMetricsPublisher;
 ...
 * {@link com.dashpipe.api.k8s.PipeletJobStatusPoller} advances stages on Job success and
...
public class PipelineRunOrchestrator {
  ...
  /**
   * Force-stops a non-terminal execution: marks {@code CANCELLED}, deletes Jobs, purges stage
   * queues so the next Run starts clean.
   */
  @Transactional
  public PipelineExecution cancel(PipelineExecution execution, int stageCount) {
    if (execution == null) {
    ...
    try {
      ...
      log.warn(
          "Job cleanup failed during cancel of {}: {}", saved.getId(), ex.getMessage());
    }

    int stages = Math.max(1, stageCount);
    try {
      PipelineTopology topology =
          topologyService.declare(saved.getTenantId(), saved.getPipelineId(), stages);
      topologyService.purgeStageQueues(topology);
    } catch (Exception ex) {
      log.warn(
          "Queue purge failed during cancel of {}: {}", saved.getId(), ex.getMessage());
    }

    log.info("Cancelled execution {} for pipeline {}", saved.getId(), saved.getPipelineId());
    return saved;
  }

...
#
# Usage:
#   ./dashpipe-ci_cd/scripts/localdev.sh                 # start (default)
#   ./dashpipe-ci_cd/scripts/localdev.sh start           # same
#   ./dashpipe-ci_cd/scripts/localdev.sh start --with-metrics --with-elk
#   ./dashpipe-ci_cd/scripts/localdev.sh start --k8s     # API profiles local,k8s + build pipelet images
#   ./dashpipe-ci_cd/scripts/localdev.sh build-pipelets  # build dashpipe/plet-*:local images only
#   ./dashpipe-ci_cd/scripts/localdev.sh stop
#   ./dashpipe-ci_cd/scripts/localdev.sh status
#   ./dashpipe-ci_cd/scripts/localdev.sh logs            # tail API + UI logs
#
# Guide: dashpipe-platform/docs/LOCALDEV_PIPELINE_GUIDE.md
#
# Env overrides:
...
#   PIPELET_IDS  space-separated pipelet dirs under pipelets/ (default: catalog below)
#
set -euo pipefail
...
parse_args() {
  if [[ $# -gt 0 ]]; then
    case "$1" in
      start|stop|status|logs|build-pipelets|help|-h|--help)
        CMD="$1"
        shift
        ;;
    esac
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --with-elk) WITH_ELK=1 ;;
      --k8s) WITH_K8S=1; BUILD_PIPELETS=1 ;;
      --build-pipelets) BUILD_PIPELETS=1 ;;
      --no-build-pipelets) BUILD_PIPELETS=0 ;;
      --no-api) NO_API=1 ;;
      --no-ui) NO_UI=1 ;;
      --no-seed) SEED_S3=0 ;;
      --seed-s3) SEED_S3=1 ;;
...
pipelet_ids() {
  if [[ -n "${PIPELET_IDS:-}" ]]; then
    # shellcheck disable=SC2206
    local ids=(${PIPELET_IDS})
    printf '%s\n' "${ids[@]}"
    return
  fi
  printf '%s\n' "${DEFAULT_PIPELET_IDS[@]}"
...
build_pipelets() {
  ...
  log "Building pipelet images (tag dashpipe/<id>:local) into Docker store used by Rancher"
  while IFS= read -r id; do
    ...
    log "docker build $id"
    if ! docker build -f "$dockerfile" -t "dashpipe/${id}:local" "$PLATFORM/pipelets"; then
      printf 'ERROR: failed to build dashpipe/%s:local\n' "$id" >&2
      failed=1
    fi
  done < <(pipelet_ids)
  if [[ "$failed" -ne 0 ]]; then
    die "One or more pipelet images failed to build"
  fi
  log "Pipelet images ready:"
  docker images 'dashpipe/plet-*' --format '  {{.Repository}}:{{.Tag}}' 2>/dev/null || true
}
...
start_compose() {
  ...
  local services=(mysql rabbitmq localstack petstore petstore-inventory)
  if [[ "$WITH_METRICS" -eq 1 ]]; then
  ...
  wait_http "$PETSTORE_INVENTORY_URL/health" "Petstore-Inventory" 60
...
start_api() {
  ...
  log "Starting dashpipe-api (profiles=$profiles) → $API_LOG"
  (
    cd "$PLATFORM"
    ...
    nohup bash -c '
      set -euo pipefail
      ./mvnw -pl dashpipe-api -am install -DskipTests -q
      exec ./mvnw -pl dashpipe-api spring-boot:run -Dspring-boot.run.profiles="'"$profiles"'"
    ' >"$API_LOG" 2>&1 &
    echo $! >"$API_PID_FILE"
  )
...
start_ui() {
  ...
  if [[ ! -d "$PLATFORM/dashpipe-ui/node_modules" ]]; then
    log "Installing dashpipe-ui dependencies"
    (cd "$PLATFORM/dashpipe-ui" && npm install)
  fi

  free_port_if_ours 5173
  : >"$UI_LOG"
  log "Starting dashpipe-ui (dev:api) → $UI_LOG"
  (
    cd "$PLATFORM/dashpipe-ui"
  ...
  for i in $(seq 1 20); do
    ...
    if grep -q "Cannot find native binding\|@rolldown/binding-" "$UI_LOG" 2>/dev/null; then
      log "UI missing native Vite/rolldown binding — reinstalling dashpipe-ui deps"
      stop_pid_file "$UI_PID_FILE" "UI"
      (cd "$PLATFORM/dashpipe-ui" && rm -rf node_modules && npm install)
      : >"$UI_LOG"
      (
        cd "$PLATFORM/dashpipe-ui"
        nohup env VITE_ENABLE_MSW=false npx vite >"$UI_LOG" 2>&1 &
...
────────────────────────────────────────
  ...
  Inventory  $PETSTORE_INVENTORY_URL  (GET /api/v3/inventory/items)
...
EOF
  ...
  Logs:          $STATE_DIR/
  Stop:          ./dashpipe-ci_cd/scripts/localdev.sh stop
  Status:        ./dashpipe-ci_cd/scripts/localdev.sh status
  Pipelets:      ./dashpipe-ci_cd/scripts/localdev.sh build-pipelets
────────────────────────────────────────
EOF
}
  ...
  if docker info >/dev/null 2>&1; then
    ...
    (
      ...
      docker compose --profile petstore --profile metrics --profile elk stop \
        mysql rabbitmq localstack petstore petstore-inventory prometheus grafana elasticsearch kibana logstash \
...
do_status() {
  ...
  check Inventory "$PETSTORE_INVENTORY_URL/health"
...
do_logs() {
  ...
  if [[ "$FOLLOW" -eq 1 ]]; then
    ...
    echo "(use: ./dashpipe-ci_cd/scripts/localdev.sh logs -f)"
  fi
}

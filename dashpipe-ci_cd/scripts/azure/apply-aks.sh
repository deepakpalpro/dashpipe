#!/usr/bin/env bash
# Apply empty Wave A Dashpipe stack to the current kubectl context (AKS).
...
IMAGE_PATTERN="${LOGIN_SERVER}/dashpipe/{pipeletId}:${TAG}"
...
images:
  - name: dashpipe/api
    newName: ${LOGIN_SERVER}/dashpipe/api
    newTag: "${TAG}"
  - name: dashpipe/ui
    newName: ${LOGIN_SERVER}/dashpipe/ui
    newTag: "${TAG}"
  - name: dashpipe/petstore
    newName: ${LOGIN_SERVER}/dashpipe/petstore
    newTag: "${TAG}"
  - name: dashpipe/petstore-inventory
    newName: ${LOGIN_SERVER}/dashpipe/petstore-inventory
    newTag: "${TAG}"
  - name: mysql
    newName: ${LOGIN_SERVER}/dashpipe/mysql
    newTag: "8.4"
  - name: rabbitmq
    newName: ${LOGIN_SERVER}/dashpipe/rabbitmq
    newTag: "3.13-management"
  - name: prom/prometheus
    newName: ${LOGIN_SERVER}/dashpipe/prometheus
    newTag: "v2.55.1"
  - name: grafana/grafana
    newName: ${LOGIN_SERVER}/dashpipe/grafana
    newTag: "11.3.1"
EOF
...
text = p.read_text()
old = 'PIPELINE_K8S_DEFAULT_IMAGE_PATTERN: "dashpipe/{pipeletId}:latest"'
...
kubectl -n dashpipe rollout status deploy/mysql --timeout=180s || true
kubectl -n dashpipe rollout status deploy/rabbitmq --timeout=180s || true
kubectl -n dashpipe rollout status deploy/dashpipe-api --timeout=300s || true
kubectl -n dashpipe rollout status deploy/dashpipe-ui --timeout=180s || true
kubectl -n dashpipe rollout status deploy/petstore --timeout=180s || true
kubectl -n dashpipe rollout status deploy/petstore-inventory --timeout=120s || true
kubectl -n dashpipe rollout status deploy/prometheus --timeout=120s || true
kubectl -n dashpipe rollout status deploy/grafana --timeout=120s || true

cat <<EOF

Empty Wave A applied in namespace 'dashpipe'.
...
UI (LoadBalancer — may take a minute):
  kubectl -n dashpipe get svc dashpipe-ui -w

Grafana / Prometheus (cluster-internal):
  kubectl -n dashpipe port-forward svc/grafana 3000:3000
  kubectl -n dashpipe port-forward svc/prometheus 9090:9090

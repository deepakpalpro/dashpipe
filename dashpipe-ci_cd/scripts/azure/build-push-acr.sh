...
acr_import "docker.io/library/mysql:8.4" "dashpipe/mysql:8.4"
acr_import "docker.io/library/rabbitmq:3.13-management" "dashpipe/rabbitmq:3.13-management"
acr_import "docker.io/prom/prometheus:v2.55.1" "dashpipe/prometheus:v2.55.1"
acr_import "docker.io/grafana/grafana:11.3.1" "dashpipe/grafana:11.3.1"

if [[ "$MODE" == "--all" ]]; then
  echo "==> Building all pipelet images (--all)"
  while IFS= read -r _df; do
    _p="$(basename "$(dirname "$_df")")"
    build_push "$_df" "$PLATFORM/pipelets" "dashpipe/${_p}"
  done < <(
  ...
  if [[ -f "$PLATFORM/pipelets/inventory/Dockerfile" ]]; then
    build_push "$PLATFORM/pipelets/inventory/Dockerfile" "$PLATFORM/pipelets/inventory" "dashpipe/inventory-pipelet"
  fi
else
...
Custom:
  ${LOGIN_SERVER}/dashpipe/api:${TAG}
  ${LOGIN_SERVER}/dashpipe/ui:${TAG}
  ${LOGIN_SERVER}/dashpipe/petstore:${TAG}
  ${LOGIN_SERVER}/dashpipe/petstore-inventory:${TAG}

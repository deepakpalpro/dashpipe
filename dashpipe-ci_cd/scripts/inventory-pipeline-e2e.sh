...
DEMO="$SUITE/dashpipe-demo"
CICD="$SUITE/dashpipe-ci_cd"
CSV_FILE="${CSV_FILE:-$DEMO/petstore/samples/inventory.csv}"
IMAGE="${IMAGE:-dashpipe/inventory-pipelet:local}"
...
ensure_deps() {
  ...
  if lsof -nP -iTCP:4010 -sTCP:LISTEN >/dev/null 2>&1; then
    if docker ps --format '{{.Names}}' | grep -qx df-petstore; then
      :
    else
...
run_docker_batch() {
  ...
  local net
  net="$(docker inspect df-localstack -f '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}' | head -1)"
  [[ -n "$net" ]] || die "Could not resolve compose network from df-localstack"
...
run_k8s() {
  ...
  if command -v kind >/dev/null 2>&1; then
    if ! kind get clusters 2>/dev/null | grep -qx 'dashpipe'; then
      log "Creating Kind cluster dashpipe"
      kind create cluster --name dashpipe
    fi
    log "Loading image into Kind"
    kind load docker-image "$IMAGE" --name dashpipe
  else

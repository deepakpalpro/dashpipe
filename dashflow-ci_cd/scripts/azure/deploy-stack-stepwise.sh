...
#   6  Deploy RabbitMQ (dashflow broker)
#   7  Deploy petstore on AKS
#   8  Deploy dashflow-api
#   9  Deploy dashflow-ui
...
#
# Override defaults:
#   RG=rg-dashflow LOCATION=eastus PREFIX=dfdev TAG=0.1.0 \
...
step_4_push_images() {
  ...
  echo "    Tag:    $TAG"
  echo
  echo "Portal check after: ACR → Repositories → dashflow/api, ui, petstore, mysql, rabbitmq"
  ...
  az acr import \
    ...
    --image "dashflow/rabbitmq:3.13-management" \
    --force

  echo
  echo "Pushed / imported:"
  echo "  ${server}/dashflow/api:${TAG}"
  echo "  ${server}/dashflow/ui:${TAG}"
  echo "  ${server}/dashflow/petstore:${TAG}"
  echo "  ${server}/dashflow/petstore-inventory:${TAG}"
  echo "  ${server}/dashflow/mysql:8.4"
  echo "  ${server}/dashflow/rabbitmq:3.13-management"
...
step_5_mysql() {
  ...
  sed "s|image: mysql:8.4|image: ${server}/dashflow/mysql:8.4|" \
...
step_6_rabbitmq() {
  echo "==> STEP 6: Deploy RabbitMQ (dashflow stage broker)"
  echo "    Namespace: $NAMESPACE"
  ...
  sed "s|image: rabbitmq:3.13-management|image: ${server}/dashflow/rabbitmq:3.13-management|" \
...
step_7_petstore() {
  ...
  sed "s|image: dashflow/petstore:latest|image: ${server}/dashflow/petstore:${TAG}|" \
...
step_8_api() {
  echo "==> STEP 8: Deploy dashflow-api"
  echo "    Namespace: $NAMESPACE"
  echo "    Depends on: MySQL (step 5) + RabbitMQ (step 6)"
  echo
  echo "Portal check after: AKS → Workloads → dashflow-api"

  local server workdir image_pattern
  server="$(login_server)"
  image_pattern="${server}/dashflow/{pipeletId}:${TAG}"
  ensure_namespace_and_secrets
  ...
  sed \
    -e "s|image: dashflow/api:latest|image: ${server}/dashflow/api:${TAG}|" \
    -e "s|PIPELINE_K8S_DEFAULT_IMAGE_PATTERN: \"dashflow/{pipeletId}:latest\"|PIPELINE_K8S_DEFAULT_IMAGE_PATTERN: \"${image_pattern}\"|" \
    "$OVERLAY/api.yaml" >"$workdir/api.yaml"

  kubectl apply -f "$workdir/rbac.yaml"
  kubectl apply -f "$workdir/api.yaml"
  echo "Waiting for dashflow-api rollout…"
  kubectl -n "$NAMESPACE" rollout status deploy/dashflow-api --timeout=300s

  echo
  kubectl -n "$NAMESPACE" get deploy,svc,pods -l app=dashflow-api

  cat <<EOF

API health (port-forward):
  kubectl -n ${NAMESPACE} port-forward svc/dashflow-api 8080:8080
  curl -s http://localhost:8080/actuator/health

Pipelet Jobs will pull: ${image_pattern}
EOF

  pause "Portal / kubectl: dashflow-api Ready; health endpoint returns UP."
}

step_9_ui() {
  echo "==> STEP 9: Deploy dashflow-ui (LoadBalancer)"
  echo "    Namespace: $NAMESPACE"
  echo "    Depends on: dashflow-api (step 8) for /api proxy"
  echo
  echo "Portal check after: AKS → Workloads → dashflow-ui → Services (EXTERNAL-IP)"
  ...
  sed "s|image: dashflow/ui:latest|image: ${server}/dashflow/ui:${TAG}|" \
    "$OVERLAY/ui.yaml" >"$workdir/ui.yaml"

  kubectl apply -f "$workdir/ui.yaml"
  echo "Waiting for dashflow-ui rollout…"
  kubectl -n "$NAMESPACE" rollout status deploy/dashflow-ui --timeout=180s

  echo
  kubectl -n "$NAMESPACE" get deploy,svc,pods -l app=dashflow-ui

  cat <<EOF

UI service is type LoadBalancer — EXTERNAL-IP may take 1–2 minutes:
  kubectl -n ${NAMESPACE} get svc dashflow-ui -w

Then open http://<EXTERNAL-IP>/

Local fallback:
  kubectl -n ${NAMESPACE} port-forward svc/dashflow-ui 8088:80
  open http://localhost:8088/
EOF

  pause "Portal: dashflow-ui Ready + LoadBalancer EXTERNAL-IP assigned."
}
...
step_10_push_pipelets() {
  ...
  echo "    Tag:      $TAG"
  echo
  echo "Portal check after: ACR → Repositories → dashflow/plet-*"
  need docker
  ...
  build_push \
    ...
    pipelets \
    "dashflow/plet-rest-source"
  build_push \
    pipelets/transformer/filter/plet-python-filter/Dockerfile \
    pipelets \
    "dashflow/plet-python-filter"
  build_push \
    pipelets/destination/notify/plet-webhook-destination/Dockerfile \
    pipelets \
    "dashflow/plet-webhook-destination"

  echo
  echo "Pushed:"
  echo "  ${server}/dashflow/plet-rest-source:${TAG}"
  echo "  ${server}/dashflow/plet-python-filter:${TAG}"
  echo "  ${server}/dashflow/plet-webhook-destination:${TAG}"
  echo
  echo "API Job pattern (step 8): ${server}/dashflow/{pipeletId}:${TAG}"
...
step_11_petstore_inventory() {
  ...
  echo "    Namespace: $NAMESPACE"
  echo "    Image pushed in step 4 (dashflow/petstore-inventory)"
  echo
  ...
  sed "s|image: dashflow/petstore-inventory:latest|image: ${server}/dashflow/petstore-inventory:${TAG}|" \

#!/usr/bin/env python3
"""plet-redis-stream-source — generated runnable pipelet."""
...
def main() -> int:
    log("plet-redis-stream-source starting")
    ...
    if isinstance(payload, str) and payload.startswith("run-"):
        message = {}
    out = run(message, execution, _connector(), "plet-redis-stream-source")

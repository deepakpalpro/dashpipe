...
```bash
./dashpipe-ci_cd/scripts/inventory-pipeline-e2e.sh          # batch container
./dashpipe-ci_cd/scripts/inventory-pipeline-e2e.sh --stages # one container per pipelet
./dashpipe-ci_cd/scripts/inventory-pipeline-e2e.sh --k8s    # Job in current cluster
```

#!/usr/bin/env bash
# Tear down Dashflow Azure resources created by deploy-stack-stepwise.sh / Bicep.
#
...
#
# Override:
#   RG=rg-dashflow AKS_NAME=dfdev-aks PREFIX=dfdev \
...
Usage: $(basename "$0") [--yes] [--k8s-only] [--list] [--help]
  ...
  --k8s-only  Only remove Dashflow/tenant namespaces + cluster RBAC (keep AKS/ACR)
...
cleanup_k8s() {
  ...
  echo "==> Deleting Dashflow workloads / tenant namespaces…"
  ...
  kubectl delete clusterrolebinding dashflow-api-pipelet-jobs --ignore-not-found
  kubectl delete clusterrole dashflow-api-pipelet-jobs --ignore-not-found
